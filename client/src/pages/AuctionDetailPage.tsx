...
const AuctionDetailPage: React.FC = () => {
  ...
  const { isConnected, address, chainId } = useAppSelector(state => state.web3);
  ...
  useEffect(() => {
    if (isConnected && currentAuction && chainId !== currentAuction.chainId) {
      toast.warning(`This auction is on a different network. Please switch to ${getNetworkName(currentAuction.chainId)}.`);
    }
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-center py-20">
          ...
          <span className="text-lg text-gray-300">Loading auction details...</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="bg-gray-800 rounded-lg overflow-hidden border border-gray-700 mb-6">
            ...
            {/* Content */}
            <div className="p-6">
              <h1 className="text-2xl font-bold text-white mb-2">{currentAuction.title}</h1>
              
              <div className="flex items-center mb-4">
                <div className="mr-2 w-6 h-6 rounded-full bg-purple-600 flex items-center justify-center text-xs">
                  {currentAuction.seller.name ? currentAuction.seller.name.charAt(0) : 'S'}
                </div>
                <div>
                  <p className="text-sm text-gray-400">Created by</p>
                  <p className="text-sm font-medium text-white">
                    {currentAuction.seller.name || `${currentAuction.seller.address.substring(0, 6)}...${currentAuction.seller.address.substring(currentAuction.seller.address.length - 4)}`}
                  </p>
                </div>
              </div>
              ...
              <div className="border-b border-gray-700 mb-4">
                <div className="flex space-x-6">
                  <button 
                    className={`pb-3 px-1 font-medium ${activeTab === 'details' ? 'text-purple-400 border-b-2 border-purple-400' : 'text-gray-400 hover:text-gray-300'}`}
                    onClick={() => setActiveTab('details')}
                  >
                    Details
                  </button>
                  <button 
                    className={`pb-3 px-1 font-medium ${activeTab === 'bids' ? 'text-purple-400 border-b-2 border-purple-400' : 'text-gray-400 hover:text-gray-300'}`}
              ...
              {activeTab === 'details' ? (
                <div>
                  <p className="text-gray-300 mb-6">{currentAuction.description}</p>
                  
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                      <h3 className="text-sm text-gray-400 mb-1">Token Standard</h3>
                      <p className="text-white">{currentAuction.tokenType}</p>
                    </div>
                    <div>
                      <h3 className="text-sm text-gray-400 mb-1">Auction Type</h3>
                      <p className="text-white capitalize">{currentAuction.auctionType}</p>
                    </div>
                    <div>
                      <h3 className="text-sm text-gray-400 mb-1">Creator Address</h3>
                      <p className="text-white font-mono text-sm">{currentAuction.seller.address}</p>
                    </div>
                    <div>
                      <h3 className="text-sm text-gray-400 mb-1">Network</h3>
                      <p className="text-white">{getNetworkName(currentAuction.chainId)}</p>
                    </div>
                  </div>
        ...
        <div className="lg:col-span-1">
          ...
          <div className="bg-gray-800 rounded-lg border border-gray-700 p-4 mt-6">
            <h3 className="text-lg font-semibold text-white mb-4">About Creator</h3>
            <div className="flex items-center">
              ...
              <div>
                <h4 className="font-medium text-white">
                  {currentAuction.seller.name || `${currentAuction.seller.address.substring(0, 6)}...`}
                </h4>
            ...
            <div className="mt-4 space-y-2">
              <button className="w-full px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg flex items-center justify-center">
                <span>View Profile</span>
              </button>
              
              <button className="w-full px-4 py-2 border border-purple-500 text-purple-500 hover:bg-purple-500 hover:bg-opacity-10 rounded-lg flex items-center justify-center">
      ...
      <div className="mt-12">
        <h2 className="text-2xl font-bold text-white mb-6">Similar Auctions</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {/* You could map through related auctions here */}
          <div className="bg-gray-800 rounded-lg p-4 border border-gray-700 text-center">
            <p className="text-gray-400">Similar auctions will be shown here</p>
          </div>
        </div>
      </div>
    </div>
